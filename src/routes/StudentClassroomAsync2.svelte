<script>
    ...
    function setMyAnswerStatus() {
        answer_message = "";
        if (answers[current_problem_seq].answer.length > 0) {
            console.log(`level=${problems[current_problem_seq].problem_level}`);
            console.log(
                `problem=${JSON.stringify(problems[current_problem_seq])}`,
            );
            console.log(
                `answer=${JSON.stringify(answers[current_problem_seq])}`,
            );
            if (problems[current_problem_seq].problem_level == 1) {
                // console.log( `0=${answers[current_problem_seq].selection_list.indexOf(0)}`,);
                // console.log( `답=${Number(answers[current_problem_seq].answer[0])}`,);
                if (
                    answers[current_problem_seq].selection_list.indexOf(0) +
                        1 ==
                    Number(answers[current_problem_seq].answer[0])
                ) {
                    answer_message = "정답입니다.";
                } else {
                    answer_message = "정답이 아닙니다.";
                }
            } else if (problems[current_problem_seq].problem_level == 2) {
                for (
                    let i = 0;
                    i < problems[current_problem_seq].level_2_num_of_answers;
                    i++
                ) {
                    let sub_prob_seq = i + 1;
                    if (sub_prob_seq == 1) {
                        let is_correct = false;
                        for (
                            let j = 0;
                            j <
                            problems[current_problem_seq].level_2_answer_1
                                .length;
                            j++
                        ) {
                            if (
                                answers[current_problem_seq].answer[i] ==
                                problems[current_problem_seq].level_2_answer_1[
                                    j
                                ]
                            ) {
                        ...
                        if (is_correct) {
                            answer_message += `문제 ${sub_prob_seq}번: 정답입니다.`;
                        } else {
                            answer_message += `문제 ${sub_prob_seq}번: 정답이 아닙니다.`;
                        }
                    } else if (sub_prob_seq == 2) {
                        let is_correct = false;
                        for (
                            let j = 0;
                            j <
                            problems[current_problem_seq].level_2_answer_2
                                .length;
                            j++
                        ) {
                            if (
                    ...
                    } else if (sub_prob_seq == 3) {
                        let is_correct = false;
                        for (
                            let j = 0;
                            j <
                            problems[current_problem_seq].level_2_answer_3
                                .length;
                            j++
                        ) {
                            if (
                    ...
                    } else if (sub_prob_seq == 4) {
                        let is_correct = false;
                        for (
                            let j = 0;
                            j <
                            problems[current_problem_seq].level_2_answer_4
                                .length;
                            j++
                        ) {
                            if (
            ...
            } else if (problems[current_problem_seq].problem_level == 3) {
                answer_message += `실습형 문제는 교사의 채점결과를 기다려주세요.`;
            }
        }
    }
    ...
    function getMyAnswerCorrection() {
        let is_disable = false;
        if (answers[current_problem_seq].answer.length > 0) {
            for (
                let i = 0;
                i < answers[current_problem_seq].answer.length;
                i++
            ) {
                if (answers[current_problem_seq].answer[i] != "") {
                    is_disable = true;
                }
            }
        } else {
...
<div class="container">
    ...
    {:then _}
        {#if myclassroom.classroom_status == 0 && is_available == true}
            ...
            <div class="text-center" style="height:27px;">
                {#if answers[current_problem_seq].answer.length > 0}
                    <div
                        class="alert alert-warning"
                        role="alert"
                        style="color:red; padding:0; font-size:12px;"
                    >
                        {answer_message}
                    </div>
                {/if}
            ...
            >
                ...
                <div>
                    <button
                        ...
                        style="display:inline-block;"
                        on:click={checkMyAnswer}
                        disabled={answers[current_problem_seq].answer.length >
                            0 &&
                            !(problems[current_problem_seq].problem_level == 3)}
                        >{current_problem_seq}번문제 정답확인</button
                    >
                    <button
                        class="btn btn-primary"
                        style="display:inline-block;"
